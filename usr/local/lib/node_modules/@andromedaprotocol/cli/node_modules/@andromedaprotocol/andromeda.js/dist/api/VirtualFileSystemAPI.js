...
class VirtualFileSystemAPI extends ADOAPI_1.default {
     ...
     * @returns
     */
    addParentPathMsg(name, parent_address) {
        return {
            add_path: {
                name,
                parent_address,
     ...
     * @returns
     */
    async addParentPath(name, parent_address, msgParams) {
        this.preMessage();
        if (!name || name.length === 0)
            throw new Error("Cannot register an empty path");
        if (!parent_address ||
            parent_address.length === 0 ||
            !(0, __1.validateAddress)(parent_address))
            throw new Error("Cannot register an invalid address for a path within the VFS");
        return this.client.execute(this.address, this.addParentPathMsg(name, parent_address), msgParams?.fee, msgParams?.memo, msgParams?.funds);
    }
    /**
